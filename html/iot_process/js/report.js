...
//	var dept = getCookie1("organ").replace(/"/g,'');
	...
	function isempty(){
		...
		if($("#problemtype").val() == ''){
			  layer.msg("属地单位不能为空", {icon: 7});
			  return true;
		}
		if($("#profession").val() == ''){
			  layer.msg("所属专业不能为空", {icon: 7});
			  return true;
		}
		if($("#problemclass").val() == ''){
			  layer.msg("问题类别不能为空", {icon: 7});
			  return true;
		}
		if($("#rfid").val().replace(/^\s+/, '').replace(/\s+$/, '') == ''){
			  layer.msg("设备位号不能为空", {icon: 7});
			  return true;
		}
		if($("#welName").val().replace(/^\s+/, '').replace(/\s+$/, '') == ''){
			  layer.msg("问题区域不能为空", {icon: 7});
			  return true;
		}
		return false;
	}
	...
	if(piid != null && piid != ""){
	  ...
	  $("#applypeople").blur(function(){
		  ...
		  //后台校验每个用户名是否合法
			  $.ajax({
				 async: false,
				 type: "POST",
				 url: url+"iot_process/report/verifyuser",
				 data: {"userList":ch},
				 dataType: "json",
     ...
     type: 'get',  
     async: false,
     url: url+'iot_process/report/unsafe/showlist', // ajax请求路径   
     success: function(data){ 
       if(data.state == 0){
	...
	//多图片上传功能
 	...
 	//监听流程上报提交事件
	  form.on('submit(problem_report)', function(data){
		 ...
		 $.ajax({
		     type: "POST"
		     ,url: url+'iot_process/process/processPure2:10:77504'    //dfid为流程定义id（暂时就是dfid="processPure2:4:47506"）
		     ...
		     ,success: function(jsonData){
		    	...
		    	if(data != undefined || data != null){
		    		...
		    		$.ajax({
		    			type: "POST",
		    			url: url+"iot_process/report/updatepho",
		   		...
		   		success: function(data){
		   			if(data.data != null && data.data != ""){
		   				...
		   				$.ajax({
		   					 async: false
			   			     ,type: "PUT"
			   			     ,url: url+'iot_process/process/nodes/next/piid/'+piid    //piid为流程实例id
			   			     ,data: {
